...
def get_paragraphs_from_pdf_name(pdf_name):
    with sqlite3.connect('qa.db') as connection:
        ...
        cursor.execute(
            "SELECT id from documents WHERE document_name = :document_name", (pdf_name,))
        ...
        cursor.execute("SELECT paragraph_text from paragraphs WHERE document_id = :document_id", {
...
class TfIdfVector():
    ...
    def get_sorted_similarity(self, query):
        ...
        cosineSimilarities = cosine_similarity(
            query_tfidf, self.tfidf_docs, dense_output=True)
...
def get_six_answers(top_6_paragraphs, query):
    ...
    for paragraph in top_6_paragraphs:
        inputs = tokenizer(
            query, paragraph, add_special_tokens=True, return_tensors="pt")
